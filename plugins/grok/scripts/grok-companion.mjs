...
const MODEL_ALIASES = new Map([["composer", "grok-composer-2.5-fast"]]);
...
function printUsage() {
  console.log(
    [
      ...
      "  node scripts/grok-companion.mjs task [--background] [--write] [--resume-last|--resume|--fresh] [--model <model|composer>] [--effort <low|medium|high>] [--prompt-file <path>] [prompt]",
...
async function buildSetupReport(cwd, actionsTaken = []) {
  ...
  if (!grokStatus.available) {
    nextSteps.push("Install the Grok CLI and make sure `grok` is on PATH.");
  }
  if (grokStatus.available && !authStatus.loggedIn) {
    nextSteps.push("Run `grok` once in a terminal and sign in.");
  }
  ...
  return {
    ...
    grok: {
      ...grokStatus,
      detail: grokStatus.available ? grokStatus.version ?? "available" : grokStatus.detail
    },
    auth: authStatus,
...
function buildReviewPrompt(context, reviewKind, focusText) {
...
function ensureGrokAvailable(cwd) {
  ...
  if (!availability.available) {
    throw new Error("Grok CLI is not installed or not on PATH. Install it, then rerun `/grok:setup`.");
  }
}
